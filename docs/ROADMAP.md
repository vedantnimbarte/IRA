...
`wingman serve` 0.3.0 was then built and run, which corrected three guesses:
`/v1/projects` returns `{"projects": [...]}` rather than a bare array, the
event names are `wingman_core::AgentEvent` in snake case rather than the
